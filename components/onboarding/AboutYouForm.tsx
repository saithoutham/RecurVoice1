...
export function AboutYouForm({ defaults }: { defaults?: Partial<FormValues> }) {
  ...
  async function onSubmit(values: FormValues) {
    ...
    if (!response.ok) {
      setError(payload.detail ?? "Could not save your profile.");
      return;
    }
    router.push("/onboarding/comorbidity");
    router.refresh();
  }
  ...
  return (
    <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-5">
      <div className="space-y-2">
        ...
        {form.formState.errors.full_name ? <p className="text-base text-[#991B1B]">{form.formState.errors.full_name.message}</p> : null}
      </div>
      <div className="grid gap-5 md:grid-cols-2">
        ...
        <div className="space-y-2">
          ...
          <Input id="treatment_start_date" type="date" {...form.register("treatment_start_date")} />
        </div>
      </div>
      <div className="grid gap-5 md:grid-cols-2">
        <div className="space-y-2">
      ...
      <div className="grid gap-5 md:grid-cols-2">
        <div className="space-y-2">
          <Label htmlFor="oncologist_name">Oncologist name</Label>
          <Input id="oncologist_name" {...form.register("oncologist_name")} />
        </div>
        <div className="space-y-2">
          <Label htmlFor="oncologist_email">Oncologist email</Label>
          <Input id="oncologist_email" type="email" {...form.register("oncologist_email")} />
        </div>
      </div>
